        ...
        if not os.path.exists(pdb.split('.')[0] + '.ser'):
            with open(pdb.split('.')[0] + '.ser', 'w') as W:
                for sid in list(set(sidx)):
                    W.write(str(sid) + '\n')
        os.system('iskew ' + pdb.split('.')[0] + '.ser >> ' + x + '_sk.txt')
...
def bfactor_part(x):
    ni = []
    for i in glob.glob(sorted('*.pdb.csv')):
        df = pd.read_csv(x + '.csv',sep=',')
        df_pep = df[df['Chain index'] == ' B']
        for j in df_pep['Residue index'].tolist():
            ni.append('B_factor-P' + str(j))
        df_pep['new_index'] = ni
        df_pep_d = df_pep.drop(['Residue index','Chain index'],axis=1).set_index('new_index').T.rename(index = {'Predicted MD fluctuation value':str(x)})
        bf.append(df_pep_d)
    pd.concat(bf).to_csv('total_bfact.txt',sep='\t')
...
def enva_part1(y):
    ...
    proc_one = Process(target=rmsd_part,args=(y.split('_')[1],))
    proc_one.start()
    proc_one.join()
    rmsd_df = pd.read_csv(y.split('_')[1] + '_rmsd.txt',sep ='\t',header=None)
    ppp = rmsd_df[rmsd_df[2] <= 1.0][0].str.split('.').str[0] + '.pdb'
    for i in ppp:
        os.system('cp ' + i + ' ../pdbs')
    os.chdir('../')
def enva_part2(aa):
    ...
    proc_one = Process(target=rmsd_part,args=('total',))
    proc_two = Process(target=nac_sk_part,args=('total',))
    proc_three = Process(target=hh_part,args=('total',))
    proc_four = Process(target=ac_part,args=('total',))
    proc_five = Process(target=bfactor_part,args=('total',))
    ...
    proc_four.join()
    proc_five.join()
    ac.append(pd.read_csv('total_ac_ct.txt', sep='\t'))
    nac.append(pd.read_csv('total_nac.txt', sep='\t'))
    sk.append(pd.read_csv('total_sk.txt', sep='\t', header=None).fillna('1.000000'))
    hh.append(pd.read_csv('total_hh.txt', sep='\t'))
    rmsd.append(pd.read_csv('total_rmsd.txt', sep='\t', header=None))
    os.chdir('../')
