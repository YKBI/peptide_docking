...
def enva_part(y):
    ...
    proc_two = Process(target=nac_sk_part,args=(y.split('_')[1],))
    proc_three = Process(target=hh_part,args=(y.split('_')[1],))
    proc_four = Process(target=ac_part,args=(y.split('_')[1],))
    proc_one.start()
    proc_two.start()
    proc_three.start()
    proc_four.start()
